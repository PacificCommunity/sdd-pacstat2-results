...
d0 <- d_raw |>
  ...
  clean_names() |>
  mutate(indicator_number = str_squish(str_extract(indicator_full, "[0-9a-z\\.]+ ")))
...
ind_sum <- d |>
  ...
  left_join(select(filter(d_code_list, category_en == "Common hierarchical codelist for PICTs"),
                   geo_pict = id, pict = name),
            by = "geo_pict") |>
...
ind_sum <- ind_sum |>
  group_by(geo_pict) |>
  mutate(n_inds = length(unique(indicator_number[n_obs > 0])),
         pict_lab = glue("{pict} ({n_inds})")) |>
  ungroup() |>
  mutate(pict = fct_reorder(pict, -n_obs, .fun = mean),
         pict_lab = fct_reorder(pict_lab, -n_obs, .fun = mean),
         indicator_number = fct_reorder(indicator_number, -n_obs, .fun = mean)) |>
  mutate(n_obs_na = ifelse(n_obs == 0, NA, n_obs),
         n_obs_c = case_when(
           n_obs == 0 ~ "None",
           n_obs == 1 ~ "One",
           n_obs >= 2 ~ "Two or more"
...
p3 <- ind_sum |>
  ...
  labs(x = "", y = "", fill = "Number of\ntime points",
       title = glue("Around {sum_tab[1, 'prop']} of SDG - country combinations have no data."),
       subtitle = glue("A further {sum_tab[2, 'prop']} have only a single observation")) +
  scale_fill_manual(values = cat_cols)
...
png("output/indicator-summary_cat.png", 7000, 3000, res = 600, type = "cairo-png")
print(p2)
dev.off()

png("output/indicator-summary_faceted.png", 8000, 5000, res = 600, type = "cairo-png")
print(p3)
dev.off()
...
best_and_worst <- ind_sum |>
  ...
  summarise(obs_mn = mean(n_obs),
            countries1 = length(unique(pict[n_obs > 0])),
            countries2 = length(unique(pict[n_obs > 1]))) |>
  ungroup() |>
  left_join(distinct(d, indicator_number, indicator_full), 
            by = "indicator_number", multiple = "all") |>
...
tas <- read_excel("raw-data/Draft SDG indicator set for 2050 Themes.xlsx", sheet = "Sheet1")
...
p4 <- d |>
  ...
  left_join(select(filter(d_code_list, category_en == "Common hierarchical codelist for PICTs"),
            ...
            by = "geo_pict") |>
  group_by(geo_pict) |>
  mutate(n_inds = length(unique(indicator[n_obs > 0])),
         pict_lab = glue("{pict} ({n_inds})")) |>
  ungroup() |>
  mutate(pict = fct_reorder(pict, -n_obs, .fun = mean),
         pict_lab = fct_reorder(pict_lab, -n_obs, .fun = mean),
  ...
  left_join(distinct(tas, indicator = `PDH Code`, ta = `Thematic Area 2050`), 
            by = "indicator") |>
  filter(!is.na(ta)) |>
...
png("output/indicator-themes_faceted.png", 8000, 7000, res = 600, type = "cairo-png")
print(p4)
dev.off()


p5 <- ind_sum  |>
  left_join(distinct(tas, indicator_number = `SDG Label`, ta = `Thematic Area 2050`), 
            by = "indicator_number") |>
  filter(!is.na(ta)) |>
...
png("output/indicator-sdg-themes_faceted.png", 8000, 7000, res = 600, type = "cairo-png")
print(p5)
dev.off()
...
png("output/indicator-sdg-themes_faceted-spaced.png", 8500, 6000, res = 600, type = "cairo-png")
